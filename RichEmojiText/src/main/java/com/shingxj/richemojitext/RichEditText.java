...
public class RichEditText extends MentionEditText {
    ...
    private void resolveDeleteTopic() {
        ...
        for (int i = 0; i < topicList.size(); i++) { //循环遍历整个输入框的所有字符
            if ((lastPos = getText().toString().indexOf(topicList.get(i).getTopicName().replace("\b", ""), lastPos)) != -1) {
        ...
        if (topicList != null && topicList.size() > 0) {
            ...
            while (matcher.find()) {
                ...
                for (int i = 0; i < topicList.size(); i++) {
                    ...
                    if (topicModel.getTopicName().replace("\b", "").equals(mentionText.replace("\b", ""))
...
//
                    ...
                    else if ("\b".equals(deleteSb) && !deleteByEnter) {
                        delIndex = s.toString().lastIndexOf("#", start);
                        length = start - delIndex;
                    }
//
    ...
    public void resolveTopicText(TopicModel topicModel) {
        String topicName = topicModel.getTopicName() + "\b";
    ...
    public List<TopicModel> getRealTopicList() {
        ...
        for (TopicModel topicModel : topicList) {
            list.add(new TopicModel(topicModel.getTopicName().replace("#", "").replace("\b", "")
